...
RIFF installs Claude Code runtime files into projects. Codex execution happens through the configured skill/CLI path and writes normal RIFF artifacts; no `.codex/`, CommandCode, or adapter harness is installed by `riff init`. (Superseded in part by D44: Codex is no longer the default executor, Sonnet is; the no-adapter half stands.)
