...
1. Run `protocols/RECONCILE.md` § Step 0 — Sync main + reconcile stale bookkeeping, with autonomous defaults: ahead-only main pushes automatically, untracked files are noted and never asked about, tracked modifications outside `.planning/` are stashed with a DECISIONS entry. Git hygiene NEVER produces a launch question — the only Step 0 condition that stays interactive is a DIVERGED main, which blocks the launch (never launch autonomous on a diverged main). The front-load's questions are product/design/UX/scope only (step 3); the only confirmation is the single yes in step 6.
...
| Dirty tree outside `.planning/` (launch or mid-run) | AskUserQuestion stash/abort | Tracked modifications: stash + DECISIONS entry. Untracked-only: proceed, list in REPORT.md — never a question in any mode |
