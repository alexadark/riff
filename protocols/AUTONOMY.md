...
- **Release is fenced too.** `lock release --run <run-id>` removes the lock only when the token matches (exit 6 = refused: the lock belongs to another run — an old session finishing late must never delete a successor's lock). Release when the run reaches `stage: done` (single run) or when the loop stops (loop mode). `--force` exists for explicit human cleanup only. A pre-redesign `lock.json` single-file lock is still honored while live and migrated when provably dead.
