...
1. **Verify PLAN.md exists** at `.planning/phases/{id}-{slug}/PLAN.md`. If missing, run `agents/planner.md` inline (the reasoning model from `profile.yaml` `models.reasoning`). Do NOT delegate planning to the wave executor — the reasoning model plans, the executor (Sonnet workers or Codex) builds.
