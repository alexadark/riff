...
Background and design reasoning behind [`MODEL.md`](./MODEL.md). Read this when deciding whether to change the dispatch rules — not during normal execution.
...
Different model = catches Claude's blind spots. Free via GPT sub. Reasoning effort controlled inside `codex:codex-rescue` skill.
