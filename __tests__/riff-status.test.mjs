...
test('status JSON and text expose the active verification request and approval command', () => {
  ...
  const wave = {
    schema_version: 1, run: 'W-status-verification', state: 'awaiting_human', mode: 'loop', provider_override: null, selected_provider: 'codex', requested_phase_ids: [], max_phases: null, max_runs: null, waves: [{ number: 1, phase_ids: ['1'], status: 'awaiting_human' }], current: { phase_id: '1', native_phase: '1-visual-check--w-status-verification-a1', attempt: 1 }, stop_reason: 'confirmation_required:1', started_at: '2026-01-01T00:00:00Z', updated_at: '2026-01-01T00:00:00Z', phases: [{ id: '1', title: 'Visual Check', status: 'awaiting_verification', attempts: [{ attempt: 1, native_phase: '1-visual-check--w-status-verification-a1', status: 'awaiting_verification' }], verification: { status: 'pending', reason: 'confirmation_required:1', checks: request.checks, phase_metadata_sha256: request.phase_metadata_sha256, request_path: '.planning/riff-wave/request.json', request_sha256: createHash('sha256').update(requestBytes).digest('hex') } }],
  };
  ...
  expect(status.ready).not.toContain('1');
  expect(status.awaiting_human).not.toContain('1');
  ...
  const approvedStatus = projectStatus(root);
  expect(approvedStatus.active_verification).toMatchObject({ status: 'approved', approval_command: null });
  expect(approvedStatus.ready).not.toContain('1');
  expect(approvedStatus.awaiting_human).not.toContain('1');
  const approvedText = execFileSync(process.execPath, [path.resolve(import.meta.dirname, '../scripts/riff-status.mjs'), '--project-root', root], { encoding: 'utf8' });
  expect(approvedText).toContain('Next: riff wave --resume --run W-status-verification');
  expect(approvedText).not.toContain('Next: riff wave --autonomous --loop');
